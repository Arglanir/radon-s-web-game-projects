...
function lecteurXML(atester){
	...
	//on accËde aux mÈthodes DOM avec truc.doc. ...
}
...
//autres fonctions de calcul rÈpÈtitif
...
function PlateauDeJeu() {
	...
	this.nouveau = function (taillex,tailley){//crÈe un nouveau plateau
			...
			for (var ii=-1;ii<2;ii++) for(var jj=-1;jj<2;jj++)//on regarde les obstacles
					if (Math.abs(ii)+Math.abs(jj)==1){//pas diagonale
						...
						case 0:
							...
							break;
						case 2://on regarde aprËs le bord
	...
	this.caseLaPlusEloigneeDe = function (x,y,joueur){//renvoie le array(x,y) de la case du joueur la plus ÈloignÈe
	...
	this.purifie = function (joueurEnCours){//en fonction des options, 1 itÈration
					...
					switch(this.partie.options.quelTypeBord()){
					  ...
					  case 1: //on regarde pas aprËs les bords
						if (entre(0,nvy,this.tailleY-1) && entre(0,nvx,this.tailleX-1)){
							...
							if (!this.partie.options.yaExplosionJoueur() && !autreCase.getChateau() && !cetteCase.getChateau() && autreCase.vaExploser() && autreCase.getJoueur() != cetteCase.getJoueur()){//l'autre case explose de mÍme, et autre joueur : on ne traverse pas, on garde la cellule
								true;
							} else
		...
		for(var x=0;x<this.tailleX;x++) for(var y=0;y<this.tailleY;y++){
			...
			if (conquetes[y][x].length > 1){ //qui gagne la case ?
				...
				for (var nbc=0;nbc<conquetes[y][x].length;nbc++){//on cherche s'il y a une personne qui a plus de prÈtentions
